...
def apply_retention(spark, table_path, retention_days, table_name):
    try:
        if not DeltaTable.isDeltaTable(spark, table_path): return
...
def main():
    ...
    tables = {
        "bronze": [(f"{config.bronze_path}/enriched", "bronze.enriched", config.bronze_retention_days)],
        "silver": [(f"{config.silver_path}/enriched", "silver.enriched", config.silver_retention_days)],
        "gold": [(f"{config.gold_path}/daily_metrics", "gold.daily_metrics", config.gold_retention_days)]
    }
    ...
    for layer in layers:
        for path, name, days in tables.get(layer, []):
            if args.action in ["retention", "all"]: apply_retention(spark, path, days, name)
            if args.action in ["vacuum", "all"]: vacuum_table(spark, path, name)
            if args.action in ["optimize", "all"]: optimize_table(spark, path, name)
