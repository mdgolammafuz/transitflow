...
class SparkConfig:
    ...
    def __post_init__(self):
        ...
        # Dedicated folder for Spark's internal streaming state
...
def load_config() -> SparkConfig:
    ...
    config = SparkConfig(
        ...
        minio_access_key=get_optional("MINIO_ROOT_USER", "minioadmin"),
        minio_secret_key=get_optional("MINIO_ROOT_PASSWORD", "minioadmin"),
        ...
        postgres_user=get_optional("POSTGRES_USER", "transit"),
        postgres_password=get_optional("POSTGRES_PASSWORD", "transit_secure_local"),
    )
...
def create_spark_session(app_name: str):
    """Create Spark session with required S3A (MinIO), Kafka, and Delta Lake connectors."""
    from pyspark.sql import SparkSession
    config = load_config()
    
    return (SparkSession.builder
        .appName(app_name)
        # These are the 'Drivers' that allow the Spark Truck to drive to MinIO and Kafka
        ...
        # S3A Infrastructure configuration
        ...
        # Optimization: Delta Lake specific features for local performance
