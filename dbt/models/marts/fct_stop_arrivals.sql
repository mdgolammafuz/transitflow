/*
    ...
    Identity: feature_id is the source-of-truth ID for both Warehouse and Sync.
    Note: We maintain Kafka metadata to prevent the "Row Collapse" bug while 
          providing the hardened metric names for the Feature API.
*/
...
with stop_performance as (
    ...
    select 
        ...
        stddev_delay, -- Raw column from intermediate
        on_time_percentage,
        avg_dwell_time_ms,
        stop_lat,
        stop_lon,
...
final as (
    select
        -- IDENTITY: Guaranteed unique via Kafka markers + Business Keys.
        -- We DO NOT omit these; they prevent the duplicate explosion in Spark sync
        -- by providing a stable, unique anchor for the ON CONFLICT clause.
        ...
        -- METRICS: Hardened names to match Feature Store API and Spark Sync expectations.
        -- We explicitly alias stddev_delay to historical_stddev_delay here.
        ...
        cast(sp.stddev_delay as double precision) as historical_stddev_delay, -- RESTORED
        ...
        -- Kafka Metadata (The anchor for uniqueness)
        sp.kafka_partition,
        sp.kafka_offset
