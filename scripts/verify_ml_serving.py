...
Verifies:
1. ML Pipeline - training works (with Pandas DataFrames)
2. Model Registry - MLflow connection
...
def check_ml_training():
    ...
    try:
        ...
        # Create a Pandas DataFrame matching the Schema
        # We need columns like 'historical_avg_delay', 'hour_of_day', etc.
        data = {}
        for col in config.feature_columns:
            data[col] = np.random.randn(500)
        
        # Add the target column
        data[config.target_column] = np.random.randn(500) * 60 + 30
        
        df = pd.DataFrame(data)

        # Pass DataFrame
        predictor, result = train_model(config, df, delta_version=1)
...
def check_model_save_load():
    ...
    try:
        ...
        data = {}
        for col in config.feature_columns:
            data[col] = np.random.randn(100)
        data[config.target_column] = np.random.randn(100) * 60
        df = pd.DataFrame(data)
        ...
        # We need to manually pickle here since we don't have a predictor.save method
        # The Registry handles MLflow, but for local checks we verify the object is picklable
        import pickle
...
def check_prediction():
    ...
    # Use a real vehicle ID if possible, otherwise use a fallback
    vehicle_id = 1362
...
def main():
    ...
    # We ensure we are in the project root to import modules correctly
