...
def check_flink():
    ...
    try:
        ...
        jobs = json.loads(urlopen("http://localhost:8082/jobs", timeout=5).read().decode()).get('jobs', [])
...
def check_kafka():
    ...
    try:
        ...
        c = Consumer({'bootstrap.servers': KAFKA_BROKER, 'group.id': f'v-{int(time.time())}', 'auto.offset.reset': 'earliest'})
...
def check_checkpoints():
    ...
    try:
        ...
        cp = json.loads(urlopen(f"http://localhost:8082/jobs/{job_id}/checkpoints", timeout=5).read().decode())
